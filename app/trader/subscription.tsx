...
export default function SubscriptionScreen() {
  ...
  return <Screen title="Plans and payouts" subtitle="Start with your free trial, then choose the BuildMate plan that fits your business.">
    <AppCard>
    ...
    <View style={styles.grid}>{Object.entries(SUBSCRIPTION_TIERS).map(([key, tier]) => <View key={key} style={styles.plan}><AppCard><Text variant="headlineSmall">{tier.name}</Text><Text variant="headlineMedium">{key === 'free' ? tier.price : `${tier.price} after trial`}</Text>{tier.features.map((feature) => <Text key={feature}>✓ {feature}</Text>)}{key !== 'free' ? <Button mode="contained" disabled={!paymentsEnabled} onPress={() => openEndpoint('/api/stripe/subscription', { tier: key })}>{paymentsEnabled ? `Choose ${tier.name}` : 'Stripe setup in progress'}</Button> : null}</AppCard></View>)}</View>
    <AppCard><Text variant="titleLarge">Receive job payments</Text><Text>Complete Stripe Express onboarding so customer deposits and balances can be routed to your bank. BuildMate never handles raw card numbers.</Text><Button mode="contained" icon="bank" disabled={!paymentsEnabled} onPress={() => openEndpoint('/api/stripe/connect')}>{paymentsEnabled ? 'Set up Stripe payouts' : 'Stripe setup in progress'}</Button></AppCard>

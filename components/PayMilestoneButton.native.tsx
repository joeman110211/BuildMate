...
export function PayMilestoneButton({ milestoneId, onPaid }: { milestoneId: string; onPaid: () => void }) {
  ...
  async function pay() {
    try {
      ...
      const initialized = await initPaymentSheet({ merchantDisplayName: 'BuildMate', paymentIntentClientSecret: clientSecret, returnURL: 'buildmate://payment-complete', allowsDelayedPaymentMethods: false, googlePay: { merchantCountryCode: 'GB', testEnv: __DEV__ }, applePay: { merchantCountryCode: 'GB' }, style: 'alwaysLight' });

...
export default function NewQuoteScreen() {
  ...
  async function submit() {
    ...
    try { setBusy(true); setError(''); await apiFetch('/api/quotes', { method: 'POST', body: JSON.stringify({ jobId, laborCost: poundsToPence(labor), materialsCost: poundsToPence(materials), vatAmount: totals.vatAmount, depositAmount, paymentTerms: terms, notes, validUntil }) }, getToken); router.replace('/trader/dashboard'); }
  ...
  return <Screen title="Create an itemised quote" subtitle={title ?? 'Customer job'}>
    ...
    <HelperText type="error" visible={Boolean(error)}>{error}</HelperText><Button mode="contained" loading={busy} disabled={busy || totals.totalAmount <= 0 || depositTooHigh || terms.length < 5} onPress={submit}>Send quote</Button>
  </Screen>;
}

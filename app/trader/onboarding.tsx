...
export default function TraderOnboarding() {
  ...
  const valid = useMemo(() => [Boolean(businessName.trim().length >= 2 && tradeCategory && subSkills.length && postcode.trim().length >= 5), bio.trim().length >= 30, certified][step], [bio, businessName, certified, postcode, step, subSkills, tradeCategory]);
  ...
  async function save() {
    ...
    try {
      ...
      await apiFetch('/api/me', { method: 'PUT', body: JSON.stringify({ businessName, tradeCategory, subSkills, bio, postcode, radiusMiles: Number(radius), qualifications: qualificationsText.split('\n').map((x) => x.trim()).filter(Boolean), externalLinks: links, photos, selfCertified: certified }) }, getToken);

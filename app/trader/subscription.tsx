...
export default function SubscriptionScreen() {
  ...
  return <Screen title="Plans and payouts" subtitle="Every new trader gets a free trial first. Stripe subscriptions and payouts can be set up when you are ready.">
    ...
    <AppCard><Text variant="titleLarge">Receive job payments</Text><Text>Complete Stripe Express onboarding so deposits and balances can be routed to your bank. BuildPair never handles raw card numbers.</Text><Button mode="contained" icon="bank" disabled={!paymentsEnabled} onPress={() => openEndpoint('/api/stripe/connect')}>{paymentsEnabled ? 'Set up Stripe payouts' : 'Payouts paused during beta'}</Button></AppCard>

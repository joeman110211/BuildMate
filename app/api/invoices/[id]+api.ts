...
export async function PATCH(request: Request, { id }: { id: string }) {
  try {
    ...
    if (action === 'send') {
      ...
      const response = await fetch('https://api.resend.com/emails', { method: 'POST', headers: { Authorization: `Bearer ${apiKey}`, 'Content-Type': 'application/json' }, body: JSON.stringify({ from, to: [current.customerEmail], subject: `BuildPair invoice ${current.invoiceNumber}`, html: `<h1>Invoice ${escape(current.invoiceNumber)}</h1><p>For ${escape(current.customerName)}</p><table cellpadding="8" border="1" cellspacing="0"><thead><tr><th>Item</th><th>Qty</th><th>Unit</th><th>Total</th></tr></thead><tbody>${rows}</tbody></table><p>Subtotal: ${money(current.subtotal)}<br>VAT: ${money(current.vatAmount)}<br><strong>Total: ${money(current.totalAmount)}</strong><br>Deposit: ${money(current.depositAmount)}${dueText ? `<br><strong>Due: ${escape(dueText)}</strong>` : ''}</p>${current.notes ? `<p>${escape(current.notes)}</p>` : ''}<p>Sent securely via BuildPair.</p>` }) });
